...
public sealed class LanguageCompiler
{
    ...
    public VerbDescriptor DescribeVerb(Type verbType, string text, IReadOnlyList<string> synonyms, Func<IVerb?> factory)
    {
        ...
        string[] explicitCapabilities = verbType.GetCustomAttributes<RequiresCapabilityAttribute>(true)
        ...
        return new VerbDescriptor(verbType, text, synonyms, compatibilityPattern, factory)
        {
            ...
            Capabilities = MetadataConventionInference.InferCapabilities(text, patterns, explicitCapabilities, verbType),
            Traits = MetadataConventionInference.InferTraits(text, verbType)
        };
    }

    public IReadOnlyList<ConstructorDescriptor> DescribeConstructors(Type type) =>
        type.GetConstructors(BindingFlags.Public | BindingFlags.Instance)
            .Select(c => new ConstructorDescriptor(
                c,
                c.GetParameters().Select(p => DescribeParameter(type, p)).ToArray(),
                ConstructorActivatorCompiler.Compile(c)))
    ...
    private static ClauseDescriptor ToClause(ParameterDescriptor p) => new(
        p.Role!.Value,
        p.ParameterType,
        !p.IsOptional,
        p.Name,
        p.Direction,
        p.IsParams ? RoleCardinality.ZeroOrMore : (p.IsOptional ? RoleCardinality.ZeroOrOne : RoleCardinality.One),
        p.Shape.ElementType);
    ...
    {
        ClauseKind? role = InferRole(p);
        NullabilityInfo n = _nullability.Create(p);
        ...
        return new(
            ...
            p.ParameterType,
            role,
            InferDirection(verbType, p, role),
            optional,
            isParams,
            p.GetCustomAttribute<FromServicesAttribute>() != null,
            n.ReadState,
            n.WriteState,
            TypeShape.Analyze(p.ParameterType));
    }

    private static ClauseKind? InferRole(ParameterInfo p)
    {
        RoleAttribute? a = p.GetCustomAttribute<RoleAttribute>();
    ...
    private static RoleDirection InferDirection(Type t, ParameterInfo p, ClauseKind? r)
    {
        if (p.GetCustomAttribute<OutputAttribute>() != null) return RoleDirection.Output;
        if (p.GetCustomAttribute<InputOutputAttribute>() != null) return RoleDirection.InputOutput;
        if (p.GetCustomAttribute<InputAttribute>() != null) return RoleDirection.Input;
        return r == ClauseKind.What && IsFamily(t, typeof(IGet), "Get") ? RoleDirection.Output : RoleDirection.Input;
    }

    private static ClauseKind? RoleKindFor(Type d) =>
        d == typeof(IWhat<>) ? ClauseKind.What :
        d == typeof(IFrom<>) ? ClauseKind.From :
        d == typeof(ITo<>) ? ClauseKind.To :
        d == typeof(IUsing<>) ? ClauseKind.Using :
        d == typeof(IWith<>) ? ClauseKind.With :
        d == typeof(IThen<>) ? ClauseKind.Then : null;

    private static Type? InferResultType(Type t) =>
        t.GetInterfaces().FirstOrDefault(x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(IVerb<>))?.GetGenericArguments()[0];
    ...
    private static Type? InferFamilyType(Type t)
    {
        ...
        Type? marker = families.FirstOrDefault(x => x.IsAssignableFrom(t));
        if (marker != null) return marker;
        Type? current = t.BaseType;
        while (current != null && current != typeof(object))
        {
            Type candidate = current.IsGenericType ? current.GetGenericTypeDefinition() : current;
            if (KnownFamilyKeyword(candidate.Name) != null) return candidate;
    ...
    {
        if (typeof(IGet).IsAssignableFrom(t)) return "GET";
        if (typeof(ISave).IsAssignableFrom(t)) return "SAVE";
        if (typeof(ILoad).IsAssignableFrom(t)) return "LOAD";
        if (typeof(ISend).IsAssignableFrom(t)) return "SEND";
        if (typeof(IDelete).IsAssignableFrom(t)) return "DELETE";
        if (typeof(IDownload).IsAssignableFrom(t)) return "DOWNLOAD";
        if (typeof(IPost).IsAssignableFrom(t)) return "POST";
        if (typeof(ITransform).IsAssignableFrom(t)) return "TRANSFORM";
        if (typeof(ISay).IsAssignableFrom(t)) return "SAY";
    ...
    private static string? KnownFamilyKeyword(string n) => n.Split('`')[0].ToUpperInvariant() switch
    {
        "GET" => "GET",
        "SAVE" => "SAVE",
        "LOAD" => "LOAD",
    ...
    private static bool IsFamily(Type t, Type marker, string legacy)
    {
        if (marker.IsAssignableFrom(t)) return true;
        Type? current = t.BaseType;
        while (current != null && current != typeof(object))
        {
            Type candidate = current.IsGenericType ? current.GetGenericTypeDefinition() : current;
            if (candidate.Name.Split('`')[0].Equals(legacy, StringComparison.OrdinalIgnoreCase)) return true;
            current = current.BaseType;
        }
        return false;
    }
