...
public sealed record LanguageManifest(
    IReadOnlyList<object> Verbs,
    IReadOnlyList<object> Qualifiers,
...
public static class LanguageIntrospection
{
    public static LanguageManifest CreateManifest(LanguageSnapshot snapshot) => new(
        snapshot.Verbs.Select(v => (object)new
        {
            ...
            traits = v.Traits,
            clauses = v.Pattern.Clauses.Select(c => new
            {
    ...
    public static string ToJson(LanguageSnapshot snapshot, bool indented = true) =>
        JsonSerializer.Serialize(CreateManifest(snapshot), new JsonSerializerOptions { WriteIndented = indented });
    ...
    public static string ExplainVerb(LanguageSnapshot snapshot, string keyword)
    {
        ...
        return string.Join(Environment.NewLine + Environment.NewLine, overloads.Select(v =>
        {
            string signature = string.Join(" ", v.Pattern.Clauses.Select(c => $"{c.Kind.ToString().ToUpperInvariant()}<{c.ValueType.Name}>"));
