...
public sealed class LanguageSnapshot
{
        ...
        IEnumerable<ModuleDescriptor>? modules = null)
    {
        ...
        foreach (WordDescriptor word in words)
        {
            ...
            foreach (string synonym in word.Synonyms) wordMap[synonym] = word;
        }
        ...
        foreach (VerbDescriptor verb in verbs)
        {
            ...
            foreach (string synonym in verb.Synonyms) AddVerb(synonym, verb);
        }
        ...
        {
            if (!verbMap.TryGetValue(key, out List<VerbDescriptor>? set)) verbMap[key] = set = [];
            set.Add(descriptor);
        }
    }
