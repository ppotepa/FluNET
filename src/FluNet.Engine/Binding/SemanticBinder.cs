...
/// <summary>
/// Binds sentence syntax to a concrete verb overload using compiled reflection metadata.
/// Lower binding cost wins: exact CLR matches are preferred over textual resolution.
/// </summary>
public sealed class SemanticBinder
{
    ...
    public BindingResult<BoundSentence> BindSentence(SentenceNode sentence, BindingContext? context = null)
    {
        ...
        if (overloads.Count == 0)
            return Failure("FLU2001", $"Unknown verb '{sentence.Verb}'.");
        ...
        if (candidates.Count == 0)
        {
            ...
            return Failure(
                "FLU2101",
                $"No overload of '{sentence.Verb}' matches this sentence. Available: {signatures}.");
        }
        ...
        if (best.Length > 1)
        {
            return Failure(
                "FLU2102",
                $"Ambiguous '{sentence.Verb}' sentence. Matching overloads: {string.Join(", ", best.Select(x => FormatSignature(x.Verb)))}.");
        }
    ...
    public BindingResult<BoundPipeline> BindPipeline(PipelineNode pipeline, BindingContext? context = null)
    {
        ...
        {
            BindingContext sentenceContext = context with { PipelineType = pipelineType };
    ...
    {
        var remaining = sentence.Clauses
            .GroupBy(x => x.Kind)
            .ToDictionary(x => x.Key, x => new Queue<ClauseNode>(x));
        ...
        foreach (ClauseDescriptor expected in verb.Pattern.Clauses)
        {
            ...
            if (remaining.TryGetValue(expected.Kind, out Queue<ClauseNode>? queue) && queue.Count > 0)
            {
                if (repeated && expected.ElementType != null)
                {
                    ...
                    cost += collection.ConversionCost;
                }
                else
                {
            ...
            if (values.Count < minimum)
            {
                ...
                if (implicitPipeline != null)
                {
                    values.Add(implicitPipeline);
                    cost += implicitPipeline.ConversionCost;
                }
            }
        ...
        if (remaining.Values.Any(queue => queue.Count > 0))
            return null;

        ConstructorDescriptor? constructor = verb.Constructors.FirstOrDefault(x => x.RoleParameterCount > 0)
            ?? verb.Constructors.FirstOrDefault();

        return new BoundSentence(verb, constructor, roles, verb.ResultType, cost);
    }

    private BoundValue? TryBindRepeatedValues(
        Queue<ClauseNode> queue,
        ClauseDescriptor expected,
        VerbDescriptor verb,
        BindingContext context)
    {
        ClauseNode[] clauses = queue.ToArray();
        queue.Clear();
        string[] texts = clauses.Select(x => x.Value switch
        {
        ...
        if (texts.Length != clauses.Length)
            return null;
        ...
        if (!_resolvers.TryResolveMany(texts, expected.ValueType, resolution, out object? collection))
            return null;

        ExpressionNode source = clauses.Length == 1
            ? clauses[0].Value
            : new InterpolatedStringExpression(string.Join(" ", texts));

        return new(source, expected.ValueType, collection?.GetType() ?? expected.ValueType, collection, 2);
    }

    private BoundValue? TryBindValue(
        ExpressionNode expression,
        ClauseDescriptor expected,
        VerbDescriptor verb,
        BindingContext context)
    {
        if (expected.Direction == RoleDirection.Output && expression is VariableExpression output)
            return new(output, expected.ValueType, expected.ValueType, null, 0);

        if (expression is PipelineValueExpression)
            return BindKnownType(expression, context.PipelineType, expected.ValueType, null);

        if (expression is VariableExpression variable)
        {
            Type? actualType = null;
            if (context.VariableTypes != null)
                context.VariableTypes.TryGetValue(variable.Name, out actualType);
            return BindKnownType(expression, actualType, expected.ValueType, null);
        }

        if (expression is InterpolatedStringExpression interpolated && expected.ValueType == typeof(string))
            return new(interpolated, typeof(string), typeof(string), null, 0);
        ...
        if (_resolvers.TryResolve(text, expected.ValueType, resolution, out object? value))
            return new(expression, expected.ValueType, value?.GetType() ?? expected.ValueType, value, expected.ValueType == typeof(string) ? 0 : 2);
    ...
    {
        if (expected.Direction == RoleDirection.Output || context.PipelineType == null)
            return null;
    ...
    private static BoundValue? BindKnownType(ExpressionNode source, Type? actualType, Type expectedType, object? value)
    {
        ...
        if (expectedType == actualType)
            return new(source, expectedType, actualType, value, 0);

        if (expectedType.IsAssignableFrom(actualType))
            return new(source, expectedType, actualType, value, 1);

        return null;
    }

    private static BindingResult<BoundSentence> Failure(string code, string message) =>
        new(null, [Diagnostic.Error(code, message)]);

    private static string FormatSignature(VerbDescriptor verb)
    {
        string clauses = string.Join(" ", verb.Pattern.Clauses.Select(x =>
            $"{x.Kind.ToString().ToUpperInvariant()}<{FriendlyName(x.ValueType)}>"));
        return string.IsNullOrEmpty(clauses) ? verb.Text : $"{verb.Text} {clauses}";
    }
