...
/// <summary>
/// End-to-end frontend: source -> AST -> semantic binding. Execution stays a separate concern.
/// </summary>
public sealed class ClassicCompiler
{
    ...
    public ClassicCompiler(LanguageSnapshot language, ValueResolverRegistry? resolvers = null)
    {
        _parser = new ClassicParser(language);
        _binder = new SemanticBinder(language, resolvers);
    }
    ...
    public ClassicCompilation Compile(string source, BindingContext? context = null)
    {
        ...
        if (!parse.Success || parse.Script == null)
