...
public sealed class SurfaceParser
{
    ...
    private static IReadOnlyList<SurfaceStatementSyntax> ParseBlock(IReadOnlyList<LineInfo> lines, ref int cursor, int indent, ICollection<SurfaceDiagnostic> diagnostics)
    {
        ...
        while (cursor < lines.Count)
        {
            ...
            {
                string inlineValue = with.Values.Count == 1 ? with.Values[0].UnquotedText.Trim() : string.Empty;
            ...
            if (parsed is SurfaceCommandSyntax repeat && repeat.NormalizedName == "REPEAT")
            {
                if (!TryRepeatCount(repeat, diagnostics, out int count) ||
                    !TryReadChildBlock(lines, ref cursor, indent, diagnostics, repeat.Span, "REPEAT block", out IReadOnlyList<SurfaceStatementSyntax>? children))
                    continue;
                statements.Add(new SurfaceRepeatSyntax(
                    count,
                    children!,
                    SourceSpan.FromBounds(repeat.Span.Start, children![^1].Span.End))
                {
                    SentenceIndex = line.SentenceIndex
                });
                continue;
            ...
            if (parsed is SurfaceCommandSyntax whileCommand && whileCommand.NormalizedName == "WHILE")
            {
                if (!TryReadChildBlock(lines, ref cursor, indent, diagnostics, whileCommand.Span, "WHILE block", out IReadOnlyList<SurfaceStatementSyntax>? children) ||
                    !SurfaceWhileDescriptor.TryCreate(whileCommand, children!, diagnostics, out SurfaceWhileDescriptor? descriptor))
                    continue;
                statements.Add(new SurfaceWhileSyntax(
                    descriptor!,
                    SourceSpan.FromBounds(whileCommand.Span.Start, children![^1].Span.End))
                {
                    SentenceIndex = line.SentenceIndex
                });
                continue;
            ...
            if (parsed is SurfaceCommandSyntax ifCommand && ifCommand.NormalizedName is "IF" or "UNLESS")
            {
                string condition = string.Join(" ", ifCommand.Values.Select(value => value.UnquotedText)).Trim().TrimEnd(':').Trim();
                if (condition.Length == 0)
                {
                    diagnostics.Add(new SurfaceDiagnostic("FLN363", "IF requires a condition and an indented block.", ifCommand.Span));
                    continue;
                }
                if (ifCommand.NormalizedName == "UNLESS")
                    condition = $"NOT ({condition})";

                if (!TryReadChildBlock(lines, ref cursor, indent, diagnostics, ifCommand.Span, "IF block", out IReadOnlyList<SurfaceStatementSyntax>? whenTrue))
                    continue;
                ...
                if (cursor < lines.Count && lines[cursor].Indent == indent)
                {
                    ...
                    {
                        cursor++;
                        string elsePhrase = string.Join(" ", elseCommand.Values.Select(value => value.UnquotedText)).Trim();
                        if (elsePhrase.StartsWith("IF ", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            {
                                diagnostics.Add(new SurfaceDiagnostic("FLN363", "ELSE IF requires a condition and an indented block.", elseCommand.Span));
                            }
                            else if (TryReadChildBlock(lines, ref cursor, indent, diagnostics, elseCommand.Span, "ELSE IF block", out IReadOnlyList<SurfaceStatementSyntax>? elseIfChildren))
                            {
                                ...
                                if (cursor < lines.Count && lines[cursor].Indent == indent)
                                {
                                    ...
                                    {
                                        cursor++;
                                        if (TryReadChildBlock(lines, ref cursor, indent, diagnostics, finalElse.Span, "ELSE block", out IReadOnlyList<SurfaceStatementSyntax>? finalElseChildren))
                                            nestedFalse.AddRange(finalElseChildren!);
                                    }
                                }
                                SourceSpan nestedSpan = SourceSpan.FromBounds(elseCommand.Span.Start,
                                    nestedFalse.Count > 0 ? nestedFalse[^1].Span.End : elseIfChildren![^1].Span.End);
                                whenFalse = [new SurfaceIfSyntax(elseIfCondition, elseIfChildren!, nestedFalse, nestedSpan)
                                {
                                    SentenceIndex = elseLine.SentenceIndex
                                }];
                            }
                        }
                        else if (TryReadChildBlock(lines, ref cursor, indent, diagnostics, elseCommand.Span, "ELSE block", out IReadOnlyList<SurfaceStatementSyntax>? elseChildren))
                            whenFalse = elseChildren!;
                    }
                }

                int end = whenFalse.Count > 0 ? whenFalse[^1].Span.End : whenTrue![^1].Span.End;
                statements.Add(new SurfaceIfSyntax(condition, whenTrue!, whenFalse, SourceSpan.FromBounds(ifCommand.Span.Start, end))
                {
                    SentenceIndex = line.SentenceIndex
                });
                continue;
    ...
    private static bool TryRepeatCount(SurfaceCommandSyntax command, ICollection<SurfaceDiagnostic> diagnostics, out int count)
    {
        count = 0;
        if (command.Values.Count != 1)
        {
            diagnostics.Add(new SurfaceDiagnostic("FLN360", "REPEAT requires a non-negative count, for example `REPEAT 3 TIMES:`.", command.Span));
            return false;
        }
        ...
        {
            diagnostics.Add(new SurfaceDiagnostic("FLN360", "REPEAT requires a count between 0 and 10000.", command.Span));
            return false;
        }
        return true;
    }
    ...
    {
        text = NaturalSurfaceSyntax.RewriteCommand(text);
        text = text.Trim();
        if (text.EndsWith(':')) text = text[..^1].TrimEnd();
        int verbEnd = 0; while (verbEnd < text.Length && !char.IsWhiteSpace(text[verbEnd])) verbEnd++;
        ...
        ICollection<SurfaceDiagnostic> diagnostics)
    {
        ...
        for (int index = line.LeadingCharacters; index < source.Length; index++)
        {
            ...
            bool period = current == '.' && depth == 0 &&
                (index == source.Length - 1 || char.IsWhiteSpace(source[index + 1]));
            if (current != ';' && !period || depth != 0) continue;
    ...
    private static string DisplayName(SurfaceStatementSyntax statement) => statement switch
    {
        ...
        SurfacePolicyDefinitionSyntax => "policy", SurfacePolicyContextSyntax => "policy context", SurfaceTaskDefinitionSyntax => "task", SurfaceRepeatSyntax => "repeat", SurfaceWhileSyntax => "while", SurfaceIfSyntax => "if", _ => statement.GetType().Name
    };
    ...
    private sealed record LineInfo(string Text, int Start, int Indent, int LeadingCharacters, int SentenceIndex = -1);
}
