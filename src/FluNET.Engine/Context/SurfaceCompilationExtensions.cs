...
public sealed record SurfaceExecutionResult(
...
{
    public static FluNETContext CreateSurfaceContext(
    ...
    public static SurfaceCompiler GetSurfaceCompiler(this FluNETContext context)
    {
        ...
        return new SurfaceCompiler(
            ...
            context.GetService<FluNET.Language.Binding.SemanticCommandBinder>(),
            context.GetService<TypedProgramCompiler>(),
            context.GetService<TypedProgramTypeValidator>(),
            context.GetService<ExecutionPlanner>());
    }

    public static SurfaceCompilationResult CompileSurface(
        this FluNETContext context,
        string source,
        SourceSyntaxKind syntaxKind = SourceSyntaxKind.Auto) =>
        context.GetSurfaceCompiler().Compile(new SourceDocument(source, syntaxKind));

    public static async ValueTask<SurfaceExecutionResult> ExecuteSurfaceAsync(
        this FluNETContext context,
        string source,
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return new SurfaceExecutionResult(compilation, null, [], null);
        }

        List<ExecutionStepResult> steps = [];
        try
        {
            object? result = await context.GetService<ExecutionPlanExecutor>()
                .ExecuteAsync(compilation.Plan, steps, cancellationToken)
                .ConfigureAwait(false);
            return new SurfaceExecutionResult(compilation, result, steps, null);
        }
        catch (Exception exception)
        {
            return new SurfaceExecutionResult(compilation, null, steps, exception);
        }
    }
}
