...
public interface ICommandBinder<TCommand, TResult> where TCommand : class, ICommand<TResult>
...
public sealed class CommandRoute<TCommand, TResult> : ICommandRoute where TCommand : class, ICommand<TResult>
{
    ...
    { FrameId = frameId; _binder = binder ?? throw new ArgumentNullException(nameof(binder)); _handler = handler ?? throw new ArgumentNullException(nameof(handler)); }
    public FrameId? FrameId { get; }
    public bool CanHandle(BoundCommand command) { ArgumentNullException.ThrowIfNull(command); if (FrameId is { } id && command.Frame.Id != id) return false; return _binder.TryBind(command) is not null; }
    public CompiledCommand? TryCompile(BoundCommand command)
    {
        ArgumentNullException.ThrowIfNull(command); if (FrameId is { } id && command.Frame.Id != id) return null;
        TCommand? typed = _binder.TryBind(command); return typed is null ? null : new CompiledCommand(command, typed, typeof(TCommand), typeof(TResult), command.Frame.ResultTypeSymbol);
    }
    ...
    {
        if (FrameId is { } id && command.FrameId != id || command.Value is not TCommand typed) return CommandDispatchResult.NotHandled;
        return CommandDispatchResult.Handled(await _handler.HandleAsync(typed, cancellationToken).ConfigureAwait(false));
    }
...
{
    private readonly IReadOnlyList<ICommandRoute> _routes;
    private readonly IExecutionResultCache _cache;
    public CommandDispatcher(IEnumerable<ICommandRoute> routes) : this(routes, new InMemoryExecutionResultCache()) { }
    public CommandDispatcher(IEnumerable<ICommandRoute> routes, IExecutionResultCache cache)
    { _routes = routes?.ToArray() ?? throw new ArgumentNullException(nameof(routes)); _cache = cache ?? throw new ArgumentNullException(nameof(cache)); }

    public bool CanDispatch(BoundCommand command) { ArgumentNullException.ThrowIfNull(command); return MatchingRoutes(command.Frame.Id).Any(route => route.CanHandle(command)); }
    public CompiledCommand? TryCompile(BoundCommand command)
    {
        ArgumentNullException.ThrowIfNull(command);
        foreach (ICommandRoute route in MatchingRoutes(command.Frame.Id)) { CompiledCommand? compiled = route.TryCompile(command); if (compiled is not null) return compiled; }
        return null;
    }
    ...
    public async ValueTask<CommandDispatchResult> TryExecuteAsync(CompiledCommand command, CancellationToken cancellationToken = default)
    {
        ...
        ExecutionCachePolicy? policy = null; string? cacheKey = null;
        if (CommandExecutionArtifactStore.TryGetCache(command.Source, out policy))
        {
            cacheKey = CommandExecutionArtifactStore.CacheKey(command.Source);
        ...
        foreach (ICommandRoute route in MatchingRoutes(command.FrameId))
        {
            ...
            if (policy is not null) _cache.Set(cacheKey!, result.Result, policy.Ttl);
            return result;
        }
    ...
    {
        ICommandRoute[] exact = _routes.Where(route => route.FrameId is { } id && id == frameId).ToArray();
        return exact.Length > 0 ? exact : _routes.Where(route => route.FrameId is null);
    }
}
...
public static class TypedCommandServiceCollectionExtensions
{
    ...
    {
        ArgumentNullException.ThrowIfNull(services); if (frameId.IsEmpty) throw new ArgumentException("A frame id is required.", nameof(frameId));
        services.AddTransient<ICommandBinder<TCommand, TResult>, TBinder>(); services.AddTransient<ICommandHandler<TCommand, TResult>, THandler>();
        services.AddTransient<ICommandRoute>(provider => new CommandRoute<TCommand, TResult>(frameId, provider.GetRequiredService<ICommandBinder<TCommand, TResult>>(), provider.GetRequiredService<ICommandHandler<TCommand, TResult>>()));
        return services;
    }
    ...
    {
        services.AddTransient<ICommandBinder<TCommand, TResult>, TBinder>(); services.AddTransient<ICommandHandler<TCommand, TResult>, THandler>();
        services.AddTransient<ICommandRoute>(provider => new CommandRoute<TCommand, TResult>(null, provider.GetRequiredService<ICommandBinder<TCommand, TResult>>(), provider.GetRequiredService<ICommandHandler<TCommand, TResult>>())); return services;
    }
}
