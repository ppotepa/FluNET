...
public sealed class InferenceEngine(
    ResourceClassifier resources,
    FormatInference formats,
    VariableNameInference names)
{
    public InferenceEngine()
        : this(new ResourceClassifier(), new FormatInference(), new VariableNameInference())
    {
    }

    public ResourceDescriptor InferResource(
        ...
        InferenceTrace? trace = null)
    {
        ...
        ResourceReference reference = resources.Classify(value);
        ResourceFormat format = formats.Infer(reference);
