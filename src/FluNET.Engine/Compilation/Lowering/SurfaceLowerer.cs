...
public sealed class SurfaceLowerer
{
    ...
    public SurfaceLowerer(InferenceEngine inference) => _inference = inference ?? throw new ArgumentNullException(nameof(inference));

    public LoweringResult Lower(SurfaceParseResult parse, PromptGrammar grammar, LanguageSnapshot? language = null)
    {
        ...
        List<SurfaceDiagnostic> diagnostics)
    {
        ...
        foreach (SurfaceStatementSyntax statement in statements)
        {
            if (statement is SurfaceContextSyntax context)
            {
                ...
                {
                    diagnostics.Add(new SurfaceDiagnostic("FLN240", $"FROM context base '{context.BaseResource.Text}' is not a valid absolute or inherited URI.", context.BaseResource.Span));
                    continue;
                }
                ...
                LowerStatements(context.Statements, current with { BaseUri = baseUri }, namedBases, grammar, language, commands, links, map, trace, diagnostics);
                implicitOutput = null;
                continue;
            }
            ...
            {
                implicitOutput = LowerPipeline(pipeline, current, namedBases, grammar, language, commands, links, map, trace, diagnostics);
                continue;
            }
            ...
            if (SurfaceDataLowering.IsDataStage(command))
            {
                if (implicitOutput is null)
                {
                    diagnostics.Add(new SurfaceDiagnostic("FLN263", $"{command.NormalizedName} requires an implicit pipeline value or explicit pipeline stage.", command.Span));
                    continue;
                }
                string output = command.Alias ?? SyntheticPipelineVariable(commands.Count);
                CommandSyntax? data = SurfaceDataLowering.Lower(command, implicitOutput, output, grammar, diagnostics);
                if (data is null) continue;
                Append([data], command.Span, current, grammar, commands, links, map);
                TraceOutput(trace, command, output);
            ...
            if (effective.NormalizedName == "SAY" && effective.Values.Count == 0 && implicitOutput is not null)
            {
                ...
                trace.Add(new InferenceDecision(InferenceKind.Dependency, "implicit-pipeline-value", implicitOutput, "previous-statement-output", command.Span));
            }

            IReadOnlyList<CommandSyntax> lowered = LowerCommand(effective, current, namedBases, grammar, language, trace, diagnostics);
        ...
        List<SurfaceDiagnostic> diagnostics)
    {
        ...
        for (int stageIndex = 0; stageIndex < pipeline.Stages.Count; stageIndex++)
        {
            ...
            {
                diagnostics.Add(new SurfaceDiagnostic("FLN250", $"Directive '{stage.Name}' cannot be used as a pipeline stage.", stage.Span));
                return null;
            }
            ...
            if (SurfaceDataLowering.IsDataStage(stage))
            {
                if (value is null)
                {
                    diagnostics.Add(new SurfaceDiagnostic("FLN251", $"Pipeline stage '{stage.Name}' has no previous value to consume.", stage.Span));
                    return null;
                }
                string output = stage.Alias ?? SyntheticPipelineVariable(commands.Count);
                CommandSyntax? data = SurfaceDataLowering.Lower(stage, value, output, grammar, diagnostics);
                if (data is null) return null;
                Append([data], stage.Span, context, grammar, commands, links, map);
                TraceOutput(trace, stage, output);
                value = output;
                continue;
            }
            ...
            if (stageIndex > 0)
            {
                ...
                {
                    diagnostics.Add(new SurfaceDiagnostic("FLN252", $"Pipeline stage '{stage.Name}' does not declare an implicit input role.", stage.Span));
                    return null;
                }
            }

            IReadOnlyList<CommandSyntax> lowered = LowerCommand(effective, context, namedBases, grammar, language, trace, diagnostics);
            if (lowered.Count != 1)
            {
                diagnostics.Add(new SurfaceDiagnostic("FLN253", $"Pipeline stage '{stage.Name}' must lower to exactly one command; found {lowered.Count}.", stage.Span));
                return null;
            }
            Append(lowered, stage.Span, context, grammar, commands, links, map);
            value = ProducedVariable(effective, lowered);
            if (stageIndex < pipeline.Stages.Count - 1 && value is null)
            {
                diagnostics.Add(new SurfaceDiagnostic("FLN254", $"Pipeline stage '{stage.Name}' does not expose a value for the next stage.", stage.Span));
                return null;
            }
        }
        return value;
    ...
    private static string SyntheticPipelineVariable(int commandCount) => $"__pipe_{commandCount:D4}";
    private static SurfaceCommandSyntax InjectValue(SurfaceCommandSyntax command, string variable) =>
        command with { Values = [new SurfaceValueSyntax($"[{variable}]", command.Span)] };

    private static void TraceOutput(InferenceTrace trace, SurfaceCommandSyntax command, string output) =>
        trace.Add(new InferenceDecision(InferenceKind.VariableName, command.Name, output,
            command.Alias is null ? "synthetic-pipeline-output" : "explicit-AS", command.Span,
            command.Alias is null ? InferenceConfidence.Certain : InferenceConfidence.Explicit));
        ...
        switch (command.NormalizedName)
        {
            case "USE":
                if (command.Values.Count != 1 || string.IsNullOrWhiteSpace(command.Alias) ||
                    !Uri.TryCreate(command.Values[0].UnquotedText, UriKind.Absolute, out Uri? namedBase) || namedBase.Scheme is not ("http" or "https"))
                {
                    diagnostics.Add(new SurfaceDiagnostic("FLN241", "USE requires one absolute HTTP(S) base and an AS alias.", command.Span));
                    return true;
                }
                namedBases[command.Alias] = namedBase;
                trace.Add(new InferenceDecision(InferenceKind.Context, command.Values[0].Text, command.Alias, "named-USE-base", command.Span, InferenceConfidence.Explicit));
                return true;
            case "RETRY":
            ...
            case "TIMEOUT":
                ...
                return true;
            case "AUTH":
                diagnostics.Add(new SurfaceDiagnostic("FLN244", "AUTH is reserved for the secret/capability provider batch.", command.Span));
                return true;
            default:
                return false;
        }
        ...
        InferenceTrace trace)
    {
        ...
        SurfaceValueSyntax[] resolved = command.Values.Select(value =>
        {
            ...
            if (namedBases.TryGetValue(prefix, out Uri? named))
            {
                ...
                trace.Add(new InferenceDecision(InferenceKind.Context, value.Text, uri.ToString(), $"named-base:{prefix}", value.Span));
                return new SurfaceValueSyntax(uri.ToString(), value.Span);
            }
            if (context.BaseUri is not null && !text.StartsWith("env:", StringComparison.OrdinalIgnoreCase) && !text.StartsWith("secret:", StringComparison.OrdinalIgnoreCase) && !text.StartsWith("sql:", StringComparison.OrdinalIgnoreCase))
            {
                Uri uri = new(context.BaseUri, text);
                trace.Add(new InferenceDecision(InferenceKind.Context, value.Text, uri.ToString(), "lexical-base-uri", value.Span));
                return new SurfaceValueSyntax(uri.ToString(), value.Span);
            }
            return value;
        }).ToArray();
        ...
        foreach (SurfaceValueSyntax value in command.Values)
        {
            ...
            switch (descriptor.Reference)
            {
                case FileResourceReference:
                    result.AddRange(LowerLoad(new SurfaceCommandSyntax("LOAD", [value], command.Alias, command.Span), grammar, language, trace, diagnostics));
                    break;
                case HttpResourceReference http when descriptor.Format == ResourceFormat.Json:
                    result.Add(new CommandSyntax([
                        Token("GETHTTP", PromptTokenKind.Word, command.Span.Start, 0),
                ...
                case HttpResourceReference:
                    diagnostics.Add(new SurfaceDiagnostic("FLN233", $"Compact HTTP GET currently has a Json contract; inferred format was '{descriptor.Format}'.", value.Span));
                    break;
                case EnvironmentResourceReference environment:
                    result.Add(new CommandSyntax([
                        Token("GETENV", PromptTokenKind.Word, command.Span.Start, 0),
                        Token($"[{variable}]", PromptTokenKind.Variable, value.Span.Start, value.Span.Length),
                        Token("FROM", PromptTokenKind.Word, value.Span.Start, 0),
        ...
        InferenceTrace trace, ICollection<SurfaceDiagnostic> diagnostics)
    {
        ...
        if (command.Values.Count > 1 && command.Alias is not null)
        {
            diagnostics.Add(new SurfaceDiagnostic("FLN221", "AS on multiple explicit LOAD resources is reserved for collection/glob lowering.", command.Span));
            return [];
        }
        List<CommandSyntax> result = [];
        foreach (SurfaceValueSyntax value in command.Values)
        {
            ...
            {
                diagnostics.Add(new SurfaceDiagnostic("FLN223", $"LOAD currently accepts local files; '{descriptor.Reference.Kind}' belongs to GET/resource providers.", value.Span));
                continue;
            }
            string variable = OutputName(command, descriptor, value, trace);
            if (file.IsPattern)
            {
                ...
                {
                    diagnostics.Add(new SurfaceDiagnostic("FLN225", $"Glob LOAD currently supports JSON patterns; '{descriptor.Format}' needs a collection codec.", value.Span));
                    continue;
                }
                result.Add(new CommandSyntax([
                    Token("LOADGLOB", PromptTokenKind.Word, command.Span.Start, 0),
                    Token($"[{variable}]", PromptTokenKind.Variable, value.Span.Start, value.Span.Length),
                    Token("FROM", PromptTokenKind.Word, value.Span.Start, 0),
            ...
            {
                diagnostics.Add(new SurfaceDiagnostic("FLN224", $"LOAD cannot infer a canonical decoder for format '{descriptor.Format}'. Use an explicit canonical command.", value.Span));
                continue;
            }
            result.Add(new CommandSyntax([
                Token("LOAD", PromptTokenKind.Word, command.Span.Start, 0),
                Token(qualifier, PromptTokenKind.Word, value.Span.Start, 0),
                Token($"[{variable}]", PromptTokenKind.Variable, value.Span.Start, value.Span.Length),
                Token("FROM", PromptTokenKind.Word, value.Span.Start, 0),
                Token($"{{{file.Path}}}", PromptTokenKind.Reference, value.Span.Start, value.Span.Length)
            ], grammar));
    ...
    {
        List<PromptToken> tokens = [Token("SAY", PromptTokenKind.Word, command.Span.Start, 0)];
        foreach (SurfaceValueSyntax value in command.Values)
        {
