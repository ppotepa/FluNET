    ...
    TypedProgramTypeValidator typeValidator, ExecutionPlanner planner, IResourceProviderRegistry resourceProviders)
{
    ...
    public SurfaceCompilationResult Compile(SourceDocument document)
    {
        ...
        TypedProgram typed;
        try { typed = typedCompiler.Compile(bound); } catch (CommandCompilationException exception) { diagnostics.Add(exception.Code, CompilationPhase.Compile, exception.Message, exception.Span); return new SurfaceCompilationResult(document, parsed, lowered, diagnostics, bound, null, null, null, CompilationPhase.Compile); }
        try { typeValidator.Validate(typed); } catch (CommandCompilationException exception) { diagnostics.Add(exception.Code, CompilationPhase.TypeCheck, exception.Message, exception.Span); return new SurfaceCompilationResult(document, parsed, lowered, diagnostics, bound, typed, null, null, CompilationPhase.TypeCheck); }
        DependencyGraph graph;
        try { graph = _dependencies.Analyze(bound, lowered.CanonicalSyntax, lowered.InferenceTrace); } catch (CommandCompilationException exception) { diagnostics.Add(exception.Code, CompilationPhase.TypeCheck, exception.Message, exception.Span); return new SurfaceCompilationResult(document, parsed, lowered, diagnostics, bound, typed, null, null, CompilationPhase.TypeCheck); }
