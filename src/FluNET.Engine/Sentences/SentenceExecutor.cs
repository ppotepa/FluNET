...
>>>>>>> origin/agent/stabilize-poc-foundation
            ...
            if (verbBaseType == null)
            {
                throw new VerbNotFoundException(
                    $"No registered verb family matches '{(sentence.Root as IKeyword)?.Text ?? sentence.Root.GetType().Name}'.");
            }
            ...
            IEnumerable<VerbUsage> implementations = OrderImplementations(
                _lexicon[verbBaseType],
                sentence.Root);
            if (!implementations.Any())
            {
                throw new VerbNotFoundException($"No implementation is registered for '{verbBaseType.Name}'.");
            }
            ...
            foreach (VerbUsage impl in implementations)
            {
                ...
                if (verbInstance != null)
                {
                    ...
                    return await ExecuteVerbInstanceAsync(verbInstance, cancellationToken).ConfigureAwait(false);
                }
            }

            throw new VerbActivationException(
                $"No implementation of '{verbBaseType.Name}' can handle this command.");
        }
        ...
        private static IEnumerable<VerbUsage> OrderImplementations(
            IEnumerable<VerbUsage> implementations,
            IWord root)
        {
            string? qualifier = root.Next switch
            {
                QualifierWord word => word.Qualifier,
                LiteralWord word when word.Value.Equals("CONFIG", StringComparison.OrdinalIgnoreCase) => "CONFIG",
                _ => null
            };

            string? source = root.Find<From>()?.Next switch
            {
            ...
            bool preferConfig = qualifier == "CONFIG" ||
                root.Next is VariableWord variable && variable.VariableReference.Contains("config", StringComparison.OrdinalIgnoreCase) ||
                root.Next is LiteralWord outputLiteral && outputLiteral.Value.Equals("config", StringComparison.OrdinalIgnoreCase) ||
                string.Equals(Path.GetExtension(source), ".json", StringComparison.OrdinalIgnoreCase);

            return implementations
                .OrderBy(usage => usage.UsageName.Equals(preferConfig ? "Config" : "Text", StringComparison.OrdinalIgnoreCase) ? 0 : 1)
                .ThenBy(usage => usage.ImplementationType.FullName, StringComparer.Ordinal);
        }
        ...
        /// </summary>
        private static async ValueTask<object?> ExecuteVerbInstanceAsync(
            object verbInstance,
            CancellationToken cancellationToken)
        {
            if (verbInstance is IAsyncVerb asyncVerb)
            {
                return await asyncVerb.InvokeAsync(cancellationToken).ConfigureAwait(false);
            }
            ...
            {
                throw new VerbActivationException($"Verb '{verbType.FullName}' does not expose Invoke().");
            }

            try
            {
                return invokeMethod.Invoke(verbInstance, null);
            }
            catch (TargetInvocationException exception) when (exception.InnerException is not null)
            {
                throw exception.InnerException;
        ...
        private object ResolveWordValue(IWord word)
        {
            ...
            if (word is ReferenceWord refWord)
            {
                ...
                bool looksLikeJsonObject = refWord.Reference.TrimStart().StartsWith('"') &&
                    refWord.Reference.Contains(':');
                return looksLikeJsonObject ? refWord.RawText : refWord.Reference;
            }

            if (word is LiteralWord litWord)
            {
                string value = litWord.Value;
