...
namespace FluNET.Syntax.Verbs
{
    ...
    /// </summary>
    public class DeleteFile : Delete<string, DirectoryInfo>, IAsyncVerb
    {
        private readonly IFluNetFileSystem _fileSystem;
        /// <summary>
        /// Parameterless constructor for WordFactory discovery.
        /// </summary>
        public DeleteFile() : this(string.Empty, new DirectoryInfo("."), DefaultCapabilities.FileSystem)
        {
        }
        ...
        public DeleteFile(string what, DirectoryInfo? from) : this(what, from, DefaultCapabilities.FileSystem)
        {
        }

        public DeleteFile(string what, DirectoryInfo? from, IFluNetFileSystem fileSystem) : base(what, from)
        {
        ...
        public override Func<DirectoryInfo?, string> Act
        {
            get
            {
                return (directory) =>
                {
                    ...
                    return DeleteAsync(fullPath).GetAwaiter().GetResult();
                };
            }
        }
        ...
        public async ValueTask<object?> InvokeAsync(CancellationToken cancellationToken = default)
        {
            ...
            return await DeleteAsync(fullPath, cancellationToken).ConfigureAwait(false);
        }
        ...
        private string ResolveFullPath()
        {
            ...
            return Path.Combine((From ?? new DirectoryInfo(".")).FullName, What);
        }
        ...
        {
            if (!await _fileSystem.FileExistsAsync(fullPath, cancellationToken).ConfigureAwait(false))
            {
                return $"File not found: {fullPath}";
            }

            await _fileSystem.DeleteFileAsync(fullPath, cancellationToken).ConfigureAwait(false);
