...
namespace FluNET.Syntax.Verbs
{
    ...
    /// </summary>
    public class GetText : Get<string[], FileInfo>, IAsyncVerb
    {
        private readonly IFluNetFileSystem _fileSystem;
        /// <summary>
        /// Parameterless constructor for WordFactory discovery.
        /// </summary>
        public GetText() : this(Array.Empty<string>(), new FileInfo("temp"), DefaultCapabilities.FileSystem)
        {
        }
        ...
        public GetText(string[] what, FileInfo from) : this(what, from, DefaultCapabilities.FileSystem)
        {
        }

        public GetText(string[] what, FileInfo from, IFluNetFileSystem fileSystem) : base(what, from)
        {
        ...
        public override Func<FileInfo, string[]> Act
        {
            get
            {
                return (info) =>
                {
                    return _fileSystem.ReadAllLinesAsync(info.FullName).GetAwaiter().GetResult();
                };
            }
        }
