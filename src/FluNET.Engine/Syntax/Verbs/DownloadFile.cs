...
namespace FluNET.Syntax.Verbs
{
    ...
    /// </summary>
    public class DownloadFile : Download<FileInfo, Uri, FileInfo>, IAsyncVerb
    {
        ...
        /// </summary>
        public DownloadFile() : this(
            new FileInfo("temp"),
            new Uri("http://temp"),
            null,
            DefaultCapabilities.Http,
        ...
        public DownloadFile(FileInfo what, Uri from, FileInfo? to = null) : this(
            what,
            from,
            to,
            DefaultCapabilities.Http,
        ...
        public DownloadFile(
            ...
            FileInfo? to,
            IHttpTransport http,
            IFluNetFileSystem fileSystem) : base(what, from, to)
        {
        ...
        public override Func<Uri, FileInfo> Act
        {
            get
            {
                return (uri) =>
                {
                    return InvokeDownloadAsync(uri).GetAwaiter().GetResult();
                };
            }
        }

        public async ValueTask<object?> InvokeAsync(CancellationToken cancellationToken = default) =>
            await InvokeDownloadAsync(From, cancellationToken).ConfigureAwait(false);

        private async Task<FileInfo> InvokeDownloadAsync(
            Uri uri,
            CancellationToken cancellationToken = default)
        {
            string destinationPath = _destinationPath ?? ExtractFilename(uri);
            byte[] fileData = await _http.GetBytesAsync(uri, cancellationToken).ConfigureAwait(false);
            await _fileSystem.WriteAllBytesAsync(destinationPath, fileData, cancellationToken)
                .ConfigureAwait(false);
            Debug.WriteLine($"Downloaded {fileData.Length} bytes from {uri} to {destinationPath}");
        ...
        /// <summary>
        /// DOWNLOAD's WHAT parameter names the output; the downloaded file is
        /// returned by the operation, so activation only needs a placeholder.
        /// </summary>
        public FileInfo ResolveWhat(string value)
        {
            string safeName = string.IsNullOrWhiteSpace(value) ? "downloaded_file" : value;
            return new FileInfo(safeName);
        }
    }
}
