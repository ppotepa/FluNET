...
public sealed record ResourceProviderResult(IReadOnlyList<CommandSyntax> Commands, string? ErrorCode = null, string? ErrorMessage = null)
{
    ...
    public static ResourceProviderResult Error(string code, string message) => new([], code, message);
}
...
public interface IResourceProviderRegistry
{
    ...
    IResourceProvider? Resolve(ResourceDescriptor descriptor);
}
...
public sealed class ResourceProviderRegistry : IResourceProviderRegistry
{
    ...
    {
        _providers = new IResourceProvider[] { new FileResourceProvider(), new HttpResourceProvider(), new EnvironmentResourceProvider() }
    ...
    public ResourceProviderResult LowerRead(ResourceProviderContext context)
    {
        ...
        if (file.IsPattern)
        {
            ...
            return new([new CommandSyntax([
                Token("LOADGLOB", PromptTokenKind.Word, context.SurfaceCommand.Span.Start),
                Token($"[{context.OutputVariable}]", PromptTokenKind.Variable, context.Source.Span.Start),
                Token("FROM", PromptTokenKind.Word, context.Source.Span.Start),
                Token($"{{{file.Path}}}", PromptTokenKind.Reference, context.Source.Span.Start)
            ], context.Grammar)]);
        }
        ...
        return new([new CommandSyntax([
            Token("LOAD", PromptTokenKind.Word, context.SurfaceCommand.Span.Start),
            Token(qualifier, PromptTokenKind.Word, context.Source.Span.Start),
            Token($"[{context.OutputVariable}]", PromptTokenKind.Variable, context.Source.Span.Start),
            Token("FROM", PromptTokenKind.Word, context.Source.Span.Start),
            Token($"{{{file.Path}}}", PromptTokenKind.Reference, context.Source.Span.Start)
        ], context.Grammar)]);
...
{
    public string Id => "core.http-json";
    ...
    public ResourceProviderResult LowerRead(ResourceProviderContext context)
    {
        ...
        return new([new CommandSyntax([
            Token("GETHTTP", PromptTokenKind.Word, context.SurfaceCommand.Span.Start), Token($"[{context.OutputVariable}]", PromptTokenKind.Variable, context.Source.Span.Start),
            Token("FROM", PromptTokenKind.Word, context.Source.Span.Start), Token($"{{{uri}}}", PromptTokenKind.Reference, context.Source.Span.Start)
        ], context.Grammar)]);
    }
    private static PromptToken Token(string text, PromptTokenKind kind, int start) => new(text, kind, Math.Max(0, start), 0);
}
...
public sealed class EnvironmentResourceProvider : IResourceProvider
{
    ...
    public ResourceProviderResult LowerRead(ResourceProviderContext context)
    {
        ...
        return new([new CommandSyntax([
            Token("GETENV", PromptTokenKind.Word, context.SurfaceCommand.Span.Start), Token($"[{context.OutputVariable}]", PromptTokenKind.Variable, context.Source.Span.Start),
            Token("FROM", PromptTokenKind.Word, context.Source.Span.Start), Token($"{{{name}}}", PromptTokenKind.Reference, context.Source.Span.Start)
        ], context.Grammar)]);
    }
    private static PromptToken Token(string text, PromptTokenKind kind, int start) => new(text, kind, Math.Max(0, start), 0);
}
