...
public sealed class ResourceProviderRegistry : IResourceProviderRegistry
{
    ...
    public IResourceProvider? Resolve(ResourceDescriptor descriptor) { IResourceProvider[] matches = _providers.Where(provider => provider.CanHandle(descriptor)).ToArray(); return matches.Length switch { 0 => null, 1 => matches[0], _ => throw new LanguageDefinitionException($"Resource '{descriptor.Reference.DisplayName}' matches multiple providers: {string.Join(", ", matches.Select(item => item.Id))}.") }; }
}

public sealed class FileResourceProvider : IResourceProvider
{
    public string Id => "core.file"; public IReadOnlyList<ResourceCapability> RequiredCapabilities => [ResourceCapability.FileRead]; public bool CanHandle(ResourceDescriptor descriptor) => descriptor.Reference is FileResourceReference;
    public ResourceProviderResult LowerRead(ResourceProviderContext context)
    {
        ...
        if (file.IsPattern) { if (context.Descriptor.Format != ResourceFormat.Json) return ResourceProviderResult.Error("FLN225", $"Glob LOAD currently supports JSON patterns; '{context.Descriptor.Format}' needs a collection codec."); return new([Command("LOADGLOB", context, file.Path)]); }
        string verb = context.Descriptor.Format switch { ResourceFormat.Csv => "LOADCSV", ResourceFormat.Xml => "LOADXML", ResourceFormat.Binary => "LOADBINARY", ResourceFormat.Image => "LOADIMAGE", _ => string.Empty };
...
{
    public string Id => "core.http-json"; public IReadOnlyList<ResourceCapability> RequiredCapabilities => [ResourceCapability.NetworkRead]; public bool CanHandle(ResourceDescriptor descriptor) => descriptor.Reference is HttpResourceReference;
    public ResourceProviderResult LowerRead(ResourceProviderContext context)
    {
...
{
    public string Id => "core.environment"; public IReadOnlyList<ResourceCapability> RequiredCapabilities => [ResourceCapability.EnvironmentRead]; public bool CanHandle(ResourceDescriptor descriptor) => descriptor.Reference is EnvironmentResourceReference;
    public ResourceProviderResult LowerRead(ResourceProviderContext context) { if (context.Intent != ResourceReadIntent.Get) return ResourceProviderResult.Error("FLN223", "Environment resources belong to GET rather than LOAD."); string name = ((EnvironmentResourceReference)context.Descriptor.Reference).Name; return new([new CommandSyntax([Token("GETENV", PromptTokenKind.Word, context.SurfaceCommand.Span.Start), Token($"[{context.OutputVariable}]", PromptTokenKind.Variable, context.Source.Span.Start), Token("FROM", PromptTokenKind.Word, context.Source.Span.Start), Token($"{{{name}}}", PromptTokenKind.Reference, context.Source.Span.Start)], context.Grammar)]); }
    private static PromptToken Token(string text, PromptTokenKind kind, int start) => new(text, kind, Math.Max(0, start), 0);
}
public sealed class SecretResourceProvider : IResourceProvider
{
    public string Id => "core.secret"; public IReadOnlyList<ResourceCapability> RequiredCapabilities => [ResourceCapability.SecretRead]; public bool CanHandle(ResourceDescriptor descriptor) => descriptor.Reference is SecretResourceReference;
    public ResourceProviderResult LowerRead(ResourceProviderContext context) { if (context.Intent != ResourceReadIntent.Get) return ResourceProviderResult.Error("FLN307", "Secrets belong to GET rather than LOAD."); string name = ((SecretResourceReference)context.Descriptor.Reference).Name; return new([new CommandSyntax([Token("GETSECRET", PromptTokenKind.Word, context.SurfaceCommand.Span.Start), Token($"[{context.OutputVariable}]", PromptTokenKind.Variable, context.Source.Span.Start), Token("FROM", PromptTokenKind.Word, context.Source.Span.Start), Token($"{{{name}}}", PromptTokenKind.Reference, context.Source.Span.Start)], context.Grammar)]); }
    private static PromptToken Token(string text, PromptTokenKind kind, int start) => new(text, kind, Math.Max(0, start), 0);
}
