...
- **compact syntax** — inference-oriented source such as `LOAD post.json, todo.json; SAY "{post.title} — {todo.title}"`.
...
3. [Statement separators](statement-separators.md) — `,` as “more of the same”, `;` as a neutral statement boundary, and how these differ from `|`, `AND` and `THEN`.
4. [Canonical language reference](canonical-language.md) — explicit command syntax, variables, connectors, conditions and execution policies.
5. [Automation and desired state](automation-and-desired-state.md) — EVERY/WATCH/WHEN, scheduler APIs, durable timer state and ENSURE.
6. [Embedding and extensibility](embedding-and-extensibility.md) — C# APIs, modules, codecs, conversions, resource providers, secrets and durable stores.
7. [Architecture](architecture.md) — front ends, compiler passes, typed IR, dependency graph and the single execution runtime.
8. [Status and limitations](status-and-limitations.md) — support matrix, current gaps and the release verification gate.
9. [Durable workflows](durable-workflows.md) — workflow journals, resume semantics and durable single-host storage.
