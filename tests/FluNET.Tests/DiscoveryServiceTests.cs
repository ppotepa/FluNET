...
namespace FluNET.Tests
{
    ...
    public class DiscoveryServiceTests
    {
        ...
        public void Constructor_ShouldDiscoverWords()
        {
            ...
            TestContext.Out.WriteLine($"Total words discovered: {discoveryService.Words.Count}");
            TestContext.Out.WriteLine("\nDiscovered word types:");
            foreach (var word in discoveryService.Words.Take(20))
            {
                TestContext.Out.WriteLine($"  - {word.FullName}");
            }
        }
        ...
        public void Verbs_ShouldDiscoverApplicationVerbs()
        {
            ...
            TestContext.Out.WriteLine($"Total verbs discovered: {discoveryService.Verbs.Count}");
            TestContext.Out.WriteLine("\nDiscovered verbs:");
            foreach (var verb in discoveryService.Verbs)
            {
                TestContext.Out.WriteLine($"  - {verb.FullName}");
            }
        ...
        public void Verbs_CheckForCLIVerbs()
        {
            ...
            TestContext.Out.WriteLine($"CLI verbs discovered: {cliVerbs.Count}");
            ...
            if (cliVerbs.Any())
            {
                TestContext.Out.WriteLine("CLI verbs were discovered (CLI assembly is loaded)");
            }
            else
            {
                TestContext.Out.WriteLine("CLI verbs were not discovered (CLI assembly not loaded in test context)");
            }
        }
        ...
        public void Verbs_ShouldNotIncludeAbstractVerbs()
        {
            ...
            TestContext.Out.WriteLine($"Abstract verbs found: {abstractVerbs.Count}");
        ...
        public void Nouns_ShouldDiscoverNouns()
        {
            ...
            TestContext.Out.WriteLine($"Total nouns discovered: {discoveryService.Nouns.Count}");
            TestContext.Out.WriteLine("\nDiscovered nouns:");
            foreach (var noun in discoveryService.Nouns.Take(20))
            {
                TestContext.Out.WriteLine($"  - {noun.FullName}");
            }
        }
        ...
        public void Words_ShouldIncludeKeywords()
        {
            ...
            TestContext.Out.WriteLine($"Keywords discovered: {keywords.Count}");
            foreach (var keyword in keywords)
            {
                TestContext.Out.WriteLine($"  - {keyword.Name}");
            }
        ...
        public void Verbs_GroupedByBaseType_ShouldShowInheritance()
        {
            ...
            // Assert
            TestContext.Out.WriteLine($"Verb inheritance groups: {verbGroups.Count}");
        ...
        public void Verbs_ShouldHaveValidText()
        {
            ...
            // Assert
            TestContext.Out.WriteLine($"Verbs checked: {discoveryService.Verbs.Count}");
            TestContext.Out.WriteLine($"Verbs with invalid text: {verbsWithInvalidText.Count}");
        ...
        public void DiscoveryService_ShouldFindVerbsFromLoadedAssemblies()
        {
            ...
            // Assert
            TestContext.Out.WriteLine($"Engine verbs: {engineVerbs.Count}");
            TestContext.Out.WriteLine($"CLI verbs: {cliVerbs.Count}");

            TestContext.Out.WriteLine("\nEngine verbs:");
            foreach (var verb in engineVerbs)
            {
                TestContext.Out.WriteLine($"  - {verb.Name} from {verb.Assembly.GetName().Name}");
            }
            ...
            TestContext.Out.WriteLine("Loaded assemblies:");
            foreach (var asm in AppDomain.CurrentDomain.GetAssemblies().OrderBy(a => a.GetName().Name))
            {
                TestContext.Out.WriteLine($"  - {asm.GetName().Name}");
            }
